-- Demograficos dos ativos (so agregados): idade (exata por mes), cor/raca (dado
-- sensivel LGPD, so contagens), estado civil e UF natal (origem).
-- jsonb { age:{...}, race:{...}, marital:{...}, origin:{...} }.
...
BEGIN
  INSERT INTO public.monthly_metrics (
    ...
    pcd, apprentice, leader_dept, tenure_base, demographics, source)
  SELECT (r->>'month')::date, r->>'brand', (r->>'business_unit')::public.business_unit,
    ...
    COALESCE(r->'tenure_base','{}'::jsonb), COALESCE(r->'demographics','{}'::jsonb), 'reconstruido'
  ...
  ON CONFLICT (month, brand, source) DO UPDATE SET
    ...
    tenure_base=EXCLUDED.tenure_base, demographics=EXCLUDED.demographics, updated_at=now();
