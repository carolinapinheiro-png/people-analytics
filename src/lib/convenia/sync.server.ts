...
export async function executarSyncConvenia(
  ...
  try {
    ...
    // ------------------------------------------------------------------
    // ANTES DO `out`, PARA A PRÉVIA TAMBÉM VER
    // ------------------------------------------------------------------
    // Este bloco estava depois do `return` da prévia. Resultado: o resumo do
    // comp-ratio só aparecia na execução que GRAVA -- ou seja, a pessoa só
    // descobria se a conta fechou depois de já ter gravado.
    //
    // É o mesmo defeito do censo de custom_fields, consertado em 03/09 pela
    // mesma razão: o número de que a decisão depende não pode ficar atrás do
    // botão que a decisão autoriza.
    //
    // A gravação continua condicionada a `confirm`; o que sobe para antes é a
    // CONTA e o aviso.
    // ======================================================================
