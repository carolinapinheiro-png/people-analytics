...
// ===========================================================================
//
// Hipótese da Carolina, e ela tem razão em desconfiar: ativos e desligados
// seriam a MESMA base, mudando só o status. Se for, o problema da listagem de
// desligados -- que não traz admissão nem departamento -- desaparece, porque
// esses campos estão no registro do mesmo `id` do outro lado.
//
// Ela sugeriu cruzar por CPF ou matrícula. O `id` é melhor por dois motivos:
// vem nas duas respostas sem precisar de campo nenhum a mais, e não é dado
// pessoal -- cruzar por CPF significaria carregar CPF para nada.
//
// Este teste é caro (baixa as duas listas inteiras), então mora num botão
// separado em vez de rodar junto do diagnóstico leve.
//
// SÓ CONTAGENS SAEM DAQUI. Os registros são lidos, o `id` é extraído, e o
// resto é descartado na mesma passada. Nada é gravado.
// ===========================================================================

export interface CruzamentoEmpresa {
  empresa: string;
  ativos: number;
  desligados: number;
  /** Desligados cujo `id` aparece também na listagem de ativos. */
  encontradosNosAtivos: number;
  /** Desligados que ficariam com admissão E departamento resolvidos. */
  resolvidos: number;
  status: { valor: string; quantidade: number }[];
  erro: string | null;
}

export interface Cruzamento {
  empresas: CruzamentoEmpresa[];
  veredito: string;
  headcountReal: number | null;
  erro: string | null;
}

export const testarCruzamento = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }): Promise<Cruzamento> => {
    await authorizeAdmin(context.claims.email as string | undefined);

    const { fontesConfiguradas } = await import('@/lib/convenia/fontes');
    const { ConveniaClient } = await import('@/lib/convenia/client.server');
    const { EMPLOYEES, EMPLOYEES_DISMISSED } = await import('@/lib/convenia/paths');

    const empresas: CruzamentoEmpresa[] = [];

    for (const f of fontesConfiguradas()) {
      const linha: CruzamentoEmpresa = {
        empresa: f.empresa, ativos: 0, desligados: 0,
        encontradosNosAtivos: 0, resolvidos: 0, status: [], erro: null,
      };

      try {
        const client = ConveniaClient.paraToken(f.token!);

        type Ativo = { id?: string; status?: unknown; hiring_date?: unknown; department?: { name?: string } };
        const ativos = await client.listarTudo<Ativo>(EMPLOYEES, { porPagina: 100 });

        // Reduzimos IMEDIATAMENTE ao mínimo: id, se tem admissão, se tem área,
        // e o status. Os outros 120 campos (CPF, RG, endereço, conta bancária)
        // morrem aqui, nesta linha, sem irem para lugar nenhum.
        const porId = new Map<string, { temAdmissao: boolean; temDept: boolean }>();
        const contaStatus = new Map<string, number>();
        for (const a of ativos) {
          if (a.id) {
            porId.set(String(a.id), {
              temAdmissao: a.hiring_date != null && a.hiring_date !== '',
              temDept: !!a.department?.name,
            });
          }
          const s = a.status == null ? '(vazio)' : String(a.status);
          contaStatus.set(s, (contaStatus.get(s) ?? 0) + 1);
        }

        const deslig = await client.listarTudo<{ id?: string }>(EMPLOYEES_DISMISSED, { porPagina: 100 });

        linha.ativos = ativos.length;
        linha.desligados = deslig.length;
        for (const d of deslig) {
          const achado = d.id ? porId.get(String(d.id)) : undefined;
          if (achado) {
            linha.encontradosNosAtivos++;
            if (achado.temAdmissao && achado.temDept) linha.resolvidos++;
          }
        }
        linha.status = [...contaStatus.entries()]
          .map(([valor, quantidade]) => ({ valor, quantidade }))
          .sort((a, b) => b.quantidade - a.quantidade);
      } catch (e) {
    ...
    const totalDesl = empresas.reduce((s, e) => s + e.desligados, 0);
    const totalAchados = empresas.reduce((s, e) => s + e.encontradosNosAtivos, 0);
    const totalAtivos = empresas.reduce((s, e) => s + e.ativos, 0);

    let veredito: string;
    let headcountReal: number | null = null;
    if (!totalDesl) {
      veredito = 'Nenhum desligado voltou — não dá para testar o cruzamento.';
    } else if (totalAchados === totalDesl) {
      veredito = `Confirmado: os ${totalDesl} desligados estão TODOS na listagem de colaboradores. É a mesma base, muda só o status — então admissão e departamento de quem saiu vêm de lá, e a reconstrução sai de uma fonte só.`;
      headcountReal = totalAtivos - totalDesl;
    } else if (totalAchados === 0) {
      veredito = `Nenhum dos ${totalDesl} desligados aparece na listagem de colaboradores. São bases separadas — para ter admissão e área de quem saiu seria preciso buscar pessoa por pessoa.`;
      headcountReal = totalAtivos;
    } else {
      veredito = `${totalAchados} de ${totalDesl} desligados aparecem na listagem de colaboradores. O cruzamento parcial é o pior caso: parte do histórico vem completo e parte não, e a série ficaria certa em alguns meses e errada em outros sem nada indicar onde.`;
    }
