    ...
    <div data-pdf-block="true" className={cn('bg-card border border-border rounded-lg p-4', className)}>
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{tx(title)}</span>
          {temAjuda && (
            <Popover>
              <PopoverTrigger asChild>
                <button
                  type="button"
                  aria-label={tx("O que este gráfico responde: {0}", [title])}
              ...
              <PopoverContent align="start" className="w-[320px] space-y-2 text-sm">
                <div className="font-semibold normal-case">{tx(title)}</div>
                ...
                {a && (
                  <p className="text-muted-foreground leading-relaxed border-t border-border/60 pt-2">
                    <strong className="text-foreground">{tx("Cuidado:")}</strong> {a.cuidado}
                  </p>
                )}
              </PopoverContent>
            </Popover>
        ...
        {subtitle && (
          <span className="text-[11px] text-muted-foreground text-right shrink-0">{tx(subtitle)}</span>
        )}
      </div>
      {children}
    </div>
