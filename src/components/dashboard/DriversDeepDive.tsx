...
const fmt = (n: number | null | undefined) =>
  n == null
    ? "—"
    : Number(n).toLocaleString(numLocale(), { minimumFractionDigits: 1, maximumFractionDigits: 1 });
...
export default function DriversDeepDive({
  ...
  if (alvo && daEmpresa) {
    return (
      <ChartCard
        title={tx("Tema por tema, e o que a média esconde")}
      ajuda="temaPorTema"
        subtitle={tx(rotuloDoRecorte)}
        icon={Layers}
      >
        <p className="text-sm text-muted-foreground py-5 leading-relaxed">
          {tx("As notas por pergunta não foram carregadas com recorte de área nesta onda, então não há esta dispersão para")}{' '}
          <strong className="text-foreground">{tx(rotuloDoRecorte)}</strong>{tx(". Não é limite do dado — cada resposta traz área e pergunta juntas —, e reimportar a onda passa a trazer. Até lá fica de fora, em vez de mostrar a amplitude da empresa inteira no lugar.")}
        </p>
      </ChartCard>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <ChartCard
        title={tx("Tema por tema, e o que a média esconde")}
      ajuda="temaPorTema"
        subtitle={tx("da pergunta mais baixa à mais alta · ordenado pela amplitude{0}", [daEmpresa ? '' : ` · ${rotuloDoRecorte}`])}
        icon={Layers}
      >
          ...
          {comDispersao.map((d) => (
            <div key={d.driver} className="space-y-1">
              <button
                ...
                aria-label={tx("Ver as {0} perguntas de {1}", [d.n, d.driver])}
                className={cn(
              ...
              >
                <span className="text-xs truncate flex items-center gap-1" title={tx(d.driver)}>
                  <ChevronDown
                    className={cn(
                ...
                <span className="text-[11px] text-muted-foreground shrink-0 tabular-nums">
                  ...
                  {d.n}{" "}{tx("pergunta")}{d.n === 1 ? "" : tx("s")} · {tx(fmt(d.min))}–{tx(fmt(d.max))}{" "}{tx("· média")}{" "}
                  {tx(fmt(d.media))}
                </span>
              </button>
              <div className="relative h-4">
                ...
                <div
                  ...
                  title={tx("média {0}", [fmt(d.media)])}
                />
              </div>
              {d.amplitude >= 0.5 && (
                <p className="text-[10px] text-muted-foreground">
                  {tx("Amplitude de")}{" "}{tx(fmt(d.amplitude))}{" "}{tx("ponto")}{d.amplitude >= 2 ? tx("s") : ""}{" "}{tx("— a média esconde \"")}{tx(d.pior.question)}&quot; ({tx(fmt(d.pior.score))}).
                  {d.n === 2 &&
                    tx(" Com duas perguntas, a barra é a distância entre elas, não uma distribuição.")}
                </p>
              )}
                ...
                <ul className="mt-1 mb-2 space-y-1 border-l-2 border-border/60 pl-3">
                  {d.itens.map((q) => (
                    <li key={q.question} className="flex items-baseline gap-2 text-[11px] leading-snug">
                      ...
                      <span className="text-muted-foreground">{tx(q.question)}</span>
                    </li>
                  ))}
                </ul>
              )}
        ...
        {perguntaUnica.length > 0 && (
          <p className="text-[11px] text-muted-foreground mt-3 leading-relaxed">
            {tx("Fora da lista porque têm")}{" "}<strong>{tx("uma pergunta só")}</strong>{tx(", e uma pergunta não tem dispersão:")}{" "}
            {perguntaUnica.map((d, i) => (
              <span key={d.driver}>
                {i > 0 && (i === perguntaUnica.length - 1 ? tx(" e ") : ", ")}
                <span className="text-foreground">{tx(d.driver)}</span> ({tx(fmt(d.media))})
              </span>
            ))}
            {tx(". A nota delas continua valendo — o que não existe ali é amplitude.")}
          </p>
        )}

        <p className="text-[11px] text-muted-foreground mt-3 leading-relaxed">
          <strong>{tx("Clique num driver")}</strong>{" "}{tx("para ver as notas pergunta a pergunta, da pior para a melhor — é onde a causa costuma estar. A barra vai da pergunta mais baixa à mais alta do driver; o traço vertical é a média. Barras curtas são temas homogêneos: a média descreve bem o driver inteiro. Barras longas escondem um item ruim atrás de itens bons — nesses, agir no driver como um todo desperdiça esforço, e a pergunta apontada abaixo da barra é onde está o problema real.")}
        </p>
      </ChartCard>
      ...
      {!departamentoSelecionado && (
        <p className="text-[11px] text-muted-foreground leading-relaxed">
          <strong>{tx("O limite desta leitura:")}</strong>{" "}{tx("a associação com o eNPS passou a ser calculada dentro de cada área que tem respostas suficientes — cinco das nove em ago/26. Dá para dizer se a alavanca de Marketing é a mesma de Technology: filtre por área e a aba mostra isso logo acima. Nas quatro áreas menores a associação continua sendo a da empresa, porque uma correlação sobre 16 ou 20 respostas ordena perguntas por acaso.")}
        </p>
      )}

      <p className="text-[11px] text-muted-foreground leading-relaxed">
        {tx("E vale o de sempre:")}{" "}<strong>{tx("\"acompanha\" não é \"causa\"")}</strong>{tx(". Todas as respostas vêm da mesma pessoa no mesmo momento, e quem está satisfeito marca alto em tudo. Isso ordena os temas entre si; não promete que mexer num levanta o eNPS.")}
      </p>
    </div>
  );
}
