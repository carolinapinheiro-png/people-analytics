              ...
              <div className="grid gap-3 text-xs md:grid-cols-3">
                <div>
                  ...
                  {sug.ancoras.length === 0 ? <p className="text-muted-foreground">{tx('Nenhuma pergunta com favorável ≥ 75% acima da empresa.')}</p>
                    : sug.ancoras.map((a) => <p key={a.p.pergunta}><span className="font-semibold">{tx(a.p.driver)}</span> · {tx(a.p.pergunta)}</p>)}
                </div>
                <div>
                  <p className="mb-1 font-semibold text-red-600 dark:text-red-400">{tx('Fricções')}</p>
                  {sug.friccoes.filter(Boolean).map((f) => (
                    <p key={f!.p.pergunta}><span className="font-semibold">{f!.etiqueta}</span> · {tx(f!.p.driver)} · {tx(f!.p.pergunta)}</p>
                  ))}
                </div>
                <div>
                  <p className="mb-1 font-semibold">{tx('Populações destacadas')}</p>
                  {sug.populacoes.map(({ pop, qualificacao }) => (
                    <p key={pop.grupo + pop.segmento}>{tx(pop.segmento)}{" "}{tx("(n =")}{" "}{tx(fmtNum(pop.n))}) · {tx('risco')} {tx(fmtPct(pop.risco, 1))}{" "}{tx("vs")}{" "}{tx(fmtPct(pop.riscoBench, 1))} · <span className="font-semibold">{qualificacao}</span></p>
                  ))}
                </div>
              </div>
              {pri && (
                    ...
                    {([['PROTEGER (Sustentar)', pri.proteger], ['TRATAR (Prioridade)', pri.tratar], ['APROFUNDAR (Incomoda, mas não move)', pri.aprofundar]] as const).map(([rot, lista]) => (
                      <div key={rot}>
                        ...
                        {lista.length === 0 ? <p className="text-muted-foreground">—</p>
                          : lista.map((p) => <p key={p.pergunta}><span className="font-semibold">{tx(p.driver)}</span> · {tx(p.pergunta)}</p>)}
                      </div>
                    ))}
                  </div>
                </div>
